...
class Usertracking
{
  ...
  private $needed_fields = array(array('name' => 'id', 'type' => 'int', 'primary_key' => 1, 'forge_type' => 'int', 'forge_auto_increment' => TRUE),
														...
														array('name' => 'http_status', 'type' => 'string', 'forge_type' => 'varchar','forge_constraint' => '5'),
														array('name' => 'timestamp', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '20'),
														array('name' => 'client_ipv4', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '50'),
														array('name' => 'client_ipv6', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '200'),
														array('name' => 'client_platforms', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '200'),
														array('name' => 'client_browser', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '200'),
														array('name' => 'client_browser_version', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '200'),
...
return inet_ntop($addr_bin);
  ...
  public function track_this()
  {
    ...
    $this->CI->load->database('oauth_logs');
    ...
    $input_data['client_ipv4'] = $this->CI->input->server('REMOTE_ADDR');
    $input_data['client_ipv6'] = $this->IPv4To6($this->CI->input->server('REMOTE_ADDR'));
   	$input_data['http_status']=$this->CI->server->get_status_code();
    $input_data['client_user_agent'] = $this->CI->agent->agent_string();
    $input_data['client_platforms'] = $this->CI->agent->platform();
    $input_data['client_browser'] = $this->CI->agent->browser();
     $input_data['client_browser_version'] = $this->CI->agent->version();
    ...
    $this->CI->load->database('oauth_logs');
    // Fillter only http statsu <> 200
    if($this->CI->server->get_status_code()!=200&&$this->CI->server->get_status_code()!=302)
   {	 $result = $this->CI->db->insert('oauth_tracking', $input_data);
  ...
  private function check_database()
  {
    ...
    if ($this->CI->db->table_exists('oauth_tracking'))
    {
      //if the table exists, check to see if the columns are setup correctly
      $fields = $this->CI->db->field_data('oauth_tracking');
      ...
      if ($num_matched < count($this->needed_fields) && $this->configuration['auto_fix_db'] === TRUE)
      {
        ...
        $this->CI->dbforge->rename_table($db_prefix . 'oauth_tracking', 'usertracking_backup_' . time());
  ...
  private function build_database_table()
  {
    ...
    $this->CI->dbforge->create_table('oauth_tracking');
    return TRUE;
  }
